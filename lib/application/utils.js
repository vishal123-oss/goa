/**
 * Application Module Utilities
 * Utilities specific to application/core handling
 */

import { isFunction, isObject, merge, isStream } from '../shared/utils.js';
import { STATUS_CODES_WITHOUT_BODY } from '../shared/constants.js';
import { getTypeForBody, calculateContentLength } from '../response/utils.js';

/**
 * Validate middleware function
 * @param {Function} fn
 * @throws {TypeError}
 */
export function validateMiddleware(fn) {
  if (!isFunction(fn)) {
    throw new TypeError('middleware must be a function!');
  }
}

/**
 * Check if value is valid middleware
 * @param {any} fn
 * @returns {boolean}
 */
export function isValidMiddleware(fn) {
  return isFunction(fn);
}

/**
 * Merge application options with defaults
 * @param {Object} options
 * @param {Object} defaults
 * @returns {Object}
 */
export function mergeOptions(options, defaults) {
  return merge({}, defaults, options);
}

/**
 * Get environment from options or process
 * @param {Object} options
 * @returns {string}
 */
export function getEnvironment(options) {
  return options.env || process.env.NODE_ENV || 'development';
}

/**
 * Create application prototype chain
 * @param {Function} ContextClass
 * @param {Function} RequestClass
 * @param {Function} ResponseClass
 * @returns {Object}
 */
export function createPrototypes(ContextClass, RequestClass, ResponseClass) {
  return {
    context: Object.create(ContextClass.prototype),
    request: Object.create(RequestClass.prototype),
    response: Object.create(ResponseClass.prototype)
  };
}

/**
 * Create context for request
 * @param {Object} app
 * @param {Object} req
 * @param {Object} res
 * @returns {Object}
 */
export function createContext(app, req, res) {
  const context = Object.create(app.context);
  const request = context.request = Object.create(app.request);
  const response = context.response = Object.create(app.response);

  // Initialize native request/response references
  context.req = request.req = response.req = req;
  context.res = request.res = response.res = res;
  
  // Initialize context with enhanced factory
  if (typeof context.init === 'function') {
    context.init(req, res, app);
  } else {
    // Fallback for basic initialization
    context.app = app;
    context.state = {};
    context.respond = true;
    context._cookies = {};
    context._session = null;
    context._startTime = Date.now();
    context._responded = false;
  }
  
  // Set up bidirectional references
  context.app = request.app = response.app = app;
  request.ctx = response.ctx = context;
  request.response = response;
  response.request = request;

  return context;
}

/**
 * Handle response based on context
 * @param {Object} ctx
 */
export function respond(ctx) {
  try {
    // Allow bypassing response handling
    if (ctx.respond === false) return;
    if (!ctx.writable) return;

    const res = ctx.res;
    let body = ctx.body;
    const code = ctx.status;

    // Ignore body for 204, 205, 304
    if (STATUS_CODES_WITHOUT_BODY[code]) {
      body = null;
      try {
        return res.end();
      } catch (e) {
        // Ignore end errors
      }
    }

    // Status body
    if (body == null) {
      body = ctx.message || String(code);
      if (!res.headersSent) {
        try {
          ctx.type = 'text';
          ctx.length = Buffer.byteLength(body);
        } catch (e) {
          // Ignore header errors
        }
      }
      try {
        return res.end(body);
      } catch (e) {
        return;
      }
    }

    // Check if stream
    if (isStream(body)) {
      try {
        return body.pipe(res);
      } catch (e) {
        return;
      }
    }

    // Ensure content-type + content-length based on body type
    try {
      const headerType = res.getHeader ? res.getHeader('Content-Type') : undefined;
      if (!res.headersSent && !headerType) {
        const inferredType = getTypeForBody(body);
        if (inferredType) {
          res.setHeader('Content-Type', inferredType);
        }
      }
    } catch (e) {
      // Ignore header errors
    }

    try {
      if (!res.headersSent && ctx.length == null) {
        const length = calculateContentLength(body);
        if (length !== undefined) {
          ctx.length = length;
        }
      }
    } catch (e) {
      // Ignore length errors
    }

    // Buffer or string
    if (Buffer.isBuffer(body) || typeof body === 'string') {
      try {
        return res.end(body);
      } catch (e) {
        return;
      }
    }

    // JSON/object types
    try {
      body = JSON.stringify(body);
    } catch (e) {
      // JSON serialization failed, send error response
      if (!res.headersSent) {
        res.setHeader('Content-Type', 'application/json');
        res.statusCode = 500;
      }
      try {
        return res.end(JSON.stringify({ error: 'Failed to serialize response body' }));
      } catch (e) {
        return;
      }
    }

    try {
      if (!res.headersSent && ctx.length == null) {
        ctx.length = Buffer.byteLength(body);
      }
    } catch (e) {
      // Ignore length errors
    }

    try {
      res.end(body);
    } catch (e) {
      // Ignore end errors
    }
  } catch (err) {
    // Catch-all error handler for response
    try {
      const res = ctx.res;
      if (!res.headersSent) {
        res.statusCode = 500;
        res.setHeader('Content-Type', 'application/json');
        res.end(JSON.stringify({ error: 'Internal Server Error', message: err.message }));
      }
    } catch (e) {
      // Ultimate fallback - do nothing
    }
  }
}

/**
 * Default error handler
 * @param {Error} err
 * @param {Object} ctx
 * @param {boolean} silent
 */
export function handleError(err, ctx, silent) {
  if (err instanceof Error === false) {
    throw new TypeError(`non-error thrown: ${err}`);
  }

  if (404 === err.status || err.expose) return;
  if (silent) return;

  const msg = err.stack || err.toString();
  console.error(`\n${msg.replace(/^/gm, '  ')}\n`);
}

/**
 * Convert application to JSON representation
 * @param {Object} app
 * @returns {Object}
 */
export function appToJSON(app) {
  return {
    subdomainOffset: app.subdomainOffset,
    proxy: app.proxy,
    env: app.env,
    middlewareCount: app.middleware?.length || 0
  };
}

/**
 * Inspect application for debugging
 * @param {Object} app
 * @returns {Object}
 */
export function inspectApp(app) {
  return {
    subdomainOffset: app.subdomainOffset,
    proxy: app.proxy,
    env: app.env,
    middleware: app.middleware,
    keys: app.keys ? '[secure]' : undefined
  };
}
