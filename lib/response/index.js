...
export default class Response {
  ...
  set body(val) {
    ...
    const setType = !this._explicitType && !getHeader(this.res, 'content-type');
    if (setType) {
      const inferredType = getTypeForBody(val);
      const mimeType = getMimeType(inferredType);
      if (mimeType) {
        setHeader(this.res, 'Content-Type', mimeType);
      }
    }

    // Calculate and set content length (skip mutable objects)
    const length = calculateContentLength(val);
    const isStreamBody = val && typeof val.pipe === 'function';
    const isMutableObject = typeof val === 'object' && val !== null && !Buffer.isBuffer(val) && !isStreamBody;

    if (length !== undefined && (!isMutableObject || this._explicitType)) {
      this.length = length;
    }
  }
